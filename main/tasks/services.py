...
def start_container(container_name, org_name, repo_name, branch_name, docker_image, external_port, internal_port=3000, docker_network="IRIS", volumes={}, env_variables={}):
    ...
    running_instance = RunningInstance.objects.get(
        branch=branch_name, repo_name=repo_name, organisation=org_name)
    if res.returncode != 0:
...
def stop_container(branch_name, repo_name, org_name):
    ...
    res = run(["sudo", "bash", NGINX_REMOVE_CONFIG_SCRIPT, org_name,
              repo_name, branch_name], stdout=PIPE, stderr=PIPE)
    if res.returncode == 0:
...
def deploy_from_git_template(self, url, user_name, token=None, social=None, org_name=None, repo_name=None, branch_name=DEFAULT_BRANCH, internal_port=80, external_port=3000, docker_image=None, dockerfile_path=None, docker_volumes={}, docker_env_variables={}, default_branch="main", docker_network=None):
    ...
    if docker_image == None:
        ...
        if res.returncode != 0:
            ...
            write_to_log(
                logs, f"Docker image {docker_image} built successfully")
            logs.write(
                f"{datetime.datetime.now()} : Docker image built successfully\n\t\t\ttagged : {docker_image}\n")

    write_to_log(logs, f"Starting container from image : {docker_image}")
    prefix = "iris"
    container_name = f"{prefix}_{org_name}_{repo_name}_{branch_name}"
    check_container_exists = run(
        ["docker", "container", "inspect", container_name], stdout=PIPE, stderr=PIPE)
    ...
    if check_container_exists.returncode == 0:
        ...
        if res.returncode != 0:
            write_to_log(
                logs, f"Error while removing existing container : {container_name}")
            logs.close()
    ...
    if not res:
        write_to_log(
            logs, f"Error while starting container : {container_name}")
    ...
    res = run(
        ["sudo", "bash", NGINX_ADD_CONFIG_SCRIPT, str(org_name.lower()), str(
            repo_name.lower()), str(branch_name.lower()), str(external_port), ],
        stdout=PIPE,
        stderr=PIPE,
    )
    if res.returncode != 0:
        logs.write(
            f"{datetime.datetime.now()} : Error while adding nginx config\n")
        logs.close()
        return False, "Error while adding nginx config\n" + res.stderr.decode('utf-8')
    logs.write(
        f"{datetime.datetime.now()} : Nginx config added successfully\n")
    logs.write(f"\n{datetime.datetime.now()} : 🥳 Container started successfully \n\ncontainer name : {container_name}\ncontainer id : {container_id}\n")
    logs.write(
        f"Visit it on : staging-{org_name.lower()}-{repo_name.lower()}-{branch_name.lower()}.iris.nitk.ac.in\n\n")
...
def deploy_from_git(self, token, url, social, user_name, org_name, repo_name, branch_name, internal_port=3000,  external_port=None, src_code_dir=None, dest_code_dir=None, docker_image=None, volumes={}, DEFAULT_BRANCH="master"):
    ...
    try:
        ...
        f = open(log_file, 'w')
    check_db_container_exists = run(
        ["docker", "container", "inspect", db_name], stdout=PIPE, stderr=PIPE)
    if check_db_container_exists.returncode != 0:
        f.write("Starting Database Container"+"\n")
        res, msg = start_db_container(
            db_image, db_name, None, None, None, db_env_variables, "IRIS")
        f.write(msg+"\n")
    else:
    ...
    # nginx config
    res = run(
        ["sudo", "bash", NGINX_ADD_CONFIG_SCRIPT_IRIS,
            str(branch_name), str(external_port)],
        stdout=PIPE,
        stderr=PIPE,
    )
    if res.returncode != 0:
...
def get_org_and_repo_name_v2(url, social="github"):
    if social == "github":
        ...
        gitlab_match = re.match(
            r'(?:https?://)?gitlab.com/([^/]+)/([^/]+)(?:\.git)?', url)
        if gitlab_match:
            repo = url.rsplit("/", 1)[-1].replace(".git", "")
            return (gitlab_match.group(1), repo)

    elif social == "iris_git":
        iris_git_match = re.search(
            r'https://git\.iris\.nitk\.ac\.in/(.*)/(.*)\.git', url)
        if iris_git_match:
